...
class NameDwDelegate(AlignDelegate):   # Унаследован от AlignDelegate
    """Заменяет возвращаемый номер названием дня недели."""
    def __init__(self):  # Инициализация класса NameDwDelegate
        super().__init__()  # Иниц-я родительского класса QStyledItemDelegate
...
class Model(QSqlTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        if role == Qt.DisplayRole and index.column() == 5:
            ...
            # we take the first two columns, get the data, turn it to integer
            # and sum them
            # [0] at the end is necessary because pyqt returns value and a bool
            # return sum(self.data(self.index(index.row(),i)) for i inrange(2))

            numColumnStart = 3
            numColumnEnd = 4
            a = index.row()
            if index.row() < countRowsDW:  # to do
                dataStart = self.data(self.index(index.row(), numColumnStart))
                dataEnd = self.data(self.index(index.row(), numColumnEnd))
    ...
    def submit(self):
        if (self.editStrategy() == QSqlTableModel.OnRowChange or
                self.editStrategy() == QSqlTableModel.OnFieldChange):
            self.workedOvertime = []
            res = self.submitAll()
            return res
        return True
...
class Window(QDialog):
    def __init__(self):
        ...
        self.lcd.display(self.timeStartFromBD.strftime("%H:%M"))  # Нач. работы
        ...
        self.view.horizontalHeader().setStyleSheet("border-width : 0px 0px \n"
                                                   "2px 0px; \n"
                                                   "border-radius: 0px;")
        ...
        self.view.horizontalHeader(  # растяуть секцию на всю рамку
        ...
        self.view.setSpan(countRowsDW, 1, 1, 3)
        # vbox layout object
    ...
    def Time(self):
        ...
        # Обновляем отображение времени начала работы
        self.lcd.display(self.timeStartFromBD.strftime("%H:%M"))
        # Обновляем отображение времени конца работы
        self.lcd_1.display(self.endWorkTime.strftime("%H:%M"))
        if 8 <= hours < 9:
            if 0 <= minutes < 15:
    ...
    def addStartTime(self):
        ...
        if DayFromBD != DayMonthNow:
            ...
            self.view.setFixedHeight(
                (HEIGHT_ROW + 7) + HEIGHT_ROW * (countRowsDW + 1))
            self.view.setSpan(countRowsDW-1, 1, 1, 1)  # Разгруппировка
            # row, column, rowCount, columnCount
            # self.view.setSpan(countRowsDW, 1, 1, 3)
            self.view.scrollToBottom()
            print("adding row")
...
def getLastRecordDate():
    query.exec(f"SELECT day FROM {tableName} \n"
               f"WHERE id = (SELECT MAX(ID) FROM {tableName})")
    query.last()
    query.value
    startTime, = range(1)
